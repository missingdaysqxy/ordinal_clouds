...
def init_loss(logits, labels, end_points=None, losstype='ordinal'):
	...
	if end_points is not None:
		...
		# The network has only 3 layers, with the front-end being resnet_v1_50/block3
		# See the graph in tensorboard for more detailed information
		with tf.variable_scope('binary_classification_for_nodata'):
			conv_1 = slim.conv2d(end_points['resnet_v1_50/block4'], 64, [3, 3], scope='conv_1')
...
def main(_):
	...
	with slim.arg_scope(resnet_v1.resnet_arg_scope()):
		probs, end_points = resnet_v1.resnet_v1_50(batch_xs, num_classes=6, is_training=False)
		probs = tf.reshape(probs, [-1, 6])
		prediction = tf.argmax(probs, axis=-1)
		loss, binary = init_loss(probs, batch_ys, end_points, losstype=FLAGS.losstype)
		#loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=probs, labels=batch_ys)
