...
from tensorflow.contrib.slim.nets import vgg as vggnet
import numpy as np
import pandas as pd
import os, sys
...
flags.DEFINE_integer('vgg_model', 19, 'The layers batch_count of vgg: 16, 19')
flags.DEFINE_string('official_model_path', './pretrained/vgg_{}.ckpt',
                    'If \'model_to_load\' is \'pretrained\', then this file will be loaded as a pretrained model')
flags.DEFINE_string('data_dir', 'E:/qxliu/ordinal_clouds/datasets/separate_relabel/', 'Where is the input data in')
...
flags.DEFINE_string('model_to_load', 'new',
...
def _checkflags(flags):
    assert flags.vgg_model in [16, 19]
...
def _get_save_dir(flags):
    if flags.model_to_load == 'pretrained':
        mtype = 'pre' + str(flags.vgg_model)
    else:
        mtype = 'unpre' + str(flags.vgg_model)
...
def train(sess, train_op, global_step_t, loss_t, summary_t, savedir, loops, logiter, save_limit=1.2):
    '''
    Main function to train the vgg
...
def main(_):
    ...
    reader, data_count = init_img_reader(os.path.join(FLAGS.data_dir, 'train' if FLAGS.is_training else 'train')
                                         , FLAGS.batch_size, FLAGS.epoch, CLASS_LIST, shuffle=True,
    ...
    def _get_vgg():
        if FLAGS.vgg_model == 16:
            return vggnet.vgg_d
        elif FLAGS.vgg_model == 19:
            return vggnet.vgg_e
        else:
            raise NotImplementedError
    ...
    if FLAGS.is_training:
        ...
        with slim.arg_scope(vggnet.vgg_arg_scope()), tf.device(gpu_train):
            # param logits: shape [batch_size, CLASS_COUNT]
            logits, end_points = vgg(batch_xs, num_classes=CLASS_COUNT, is_training=True)
            ...
            if FLAGS.model_to_load.lower() == 'last':
                ...
                # Load the model pretrained by TensorFlow official
                exclusions = ['vgg_{}/fc8'.format(FLAGS.vgg_model), 'predictions']
                vgg_except_logits = slim.get_variables_to_restore(exclude=exclusions)
                path = FLAGS.official_model_path.format(FLAGS.vgg_model)
                init_fn = slim.assign_from_checkpoint_fn(path, vgg_except_logits,
    ...
    if not FLAGS.is_training:  # Evaluate
        with slim.arg_scope(vggnet.vgg_arg_scope()), tf.device(gpu_test):
            probs, end_points = vgg(batch_xs, num_classes=CLASS_COUNT, )
            ...
            if FLAGS.model_to_load.lower() == 'last':
                ...
                exclusions = ['vgg_{}/logits'.format(FLAGS.vgg_model), 'predictions']
                vgg_except_logits = slim.get_variables_to_restore(exclude=exclusions)
                path = FLAGS.official_model_path.format(FLAGS.vgg_model)
                init_fn = slim.assign_from_checkpoint_fn(path, vgg_except_logits,
                                                         ignore_missing_vars=True)
